...
exports.syncWithSwagger = function (swagger, options = {}) {
  var { pathsDir, componentsDir, mainFile } = calcPaths(options.basedir)

  if (_.isString(swagger)) {
    if (!dirExist(pathsDir) && !dirExist(componentsDir)) {
  ...
  if (swagger.components && dirExist(componentsDir)) {
    updateGlobObject(componentsDir, swagger.components)
    swagger = _.omit(swagger, 'components')
  }
...
exports.bundle = function (options = {}) {
  var { pathsDir, componentsDir, mainFile, codeSamplesDir } = calcPaths(options.basedir)
  ...
  if (dirExist(componentsDir)) {
    console.log('Adding components to spec')
    if (swagger.components) { throw Error('All components should be defined inside ' + componentsDir) }
    const foldersAndContents =
      'schemas,responses,parameters,examples,requestBodies,headers,securitySchemes,links,callbacks'
        .split(',')
        .map(function(folder) {
          const subFolder = componentsDir + '/' + folder
          if (dirExist(subFolder)) {
            return [folder, globYamlObject(subFolder, baseName)]
          }
        })
    swagger.components = _.fromPairs(_.compact(foldersAndContents))
...
function globObject (dir, pattern, objectPathCb) {
  return _.reduce(glob(dir + pattern), function (result, path) {
    ...
    if (_.has(result, objPath)) { throw new Error(objPath + ' component already exists') }
